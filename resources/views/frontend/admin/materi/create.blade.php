...
<div class="container-fluid">
    ...
    <form action="{{ route('admin.ruang-materi.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div>
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 small text-muted">
                        ...
                        <li class="breadcrumb-item active" aria-current="page text-dark">Buat Baru</li>
                    </ol>
                </nav>
            </div>
        ...
        <div class="row g-4">
                ...
                <div class="card border-0 shadow-sm p-4" style="background: var(--bg-card); border-radius: 20px;">
                    <div class="mb-4">
                        ...
                        <input type="text" name="judul" class="form-control form-control-lg border-0 bg-light rounded-3 px-3 @error('judul') is-invalid @enderror" 
                               placeholder="Masukkan judul materi..." value="{{ old('judul') }}" required autofocus>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="kategori" class="form-control border-0 bg-light rounded-3 px-3 py-2 @error('kategori') is-invalid @enderror" 
                               placeholder="Contoh: Tekstil, Workshop, Bisnis..." value="{{ old('kategori') }}" required>
                    </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card border-0 shadow-sm p-4 mb-4" style="background: var(--bg-card); border-radius: 20px;">
                    <label class="form-label fw-bold small text-uppercase tracking-wider mb-3">Thumbnail Preview</label>
                    
                    <div class="upload-area" onclick="document.getElementById('thumbnailInput').click()">
                        <div id="placeholderView" class="text-center p-3">
                            <i class="bi bi-image text-muted" style="font-size: 3rem;"></i>
                            <p class="small text-muted mt-2">Format: JPG, PNG, WEBP<br>(Maks. 2MB)</p>
                            <span class="btn btn-sm btn-outline-dark rounded-pill px-3">Pilih Foto</span>
                        </div>
                        <div id="previewContainer" class="d-none text-center p-2">
                            <img id="imagePreview" src="#" class="preview-img mb-3" alt="Preview">
                            <p class="small text-primary fw-bold mb-0">Ganti Foto</p>
                        </div>
                    </div>
                ...
                <div class="card border-0 shadow-sm p-4" style="background: var(--bg-card); border-radius: 20px;">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <span class="fw-bold">Publikasikan Langsung?</span>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="status" value="published" checked role="switch" style="width: 45px; height: 22px;">
                        </div>
                    </div>
                    <p class="text-muted small mb-0">Matikan saklar ini jika Anda ingin menyimpan materi sebagai <strong>Draft</strong> terlebih dahulu.</p>
                </div>
            </div>
        </div>
    </form>
...
<script>
    ...
    // Proteksi Trix dari Attachment Berat
    document.addEventListener("trix-file-accept", function(event) {
        event.preventDefault();
        alert("Lampiran file langsung tidak diizinkan. Gunakan Thumbnail untuk gambar utama.");
    });
</script>
@endsection